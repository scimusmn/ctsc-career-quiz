...
if (!fs.existsSync('src/Arduino')) {
  ...
  execSync(
    'cd src/Arduino && git submodule add https://github.com/scimusmn/arduino-base'
  );
} else {
  console.warn(
    'Arduino directory already exists. Skipping submodule installation.'
  );
  console.warn(
    'If you want a fresh re-install, delete the Arduino directory and run this script again.'
  );
  exit(1);
}
...
execSync(
  'cp src/Arduino/arduino-base/ReactSerial/examples/gatsby/gatsby-wrapper-page.js src/pages/arduino.js'
);
...
if (!fs.existsSync('src/html.js')) {
  ...
  execSync(
    "sed \"s^</body>^  <script dangerouslySetInnerHTML={{ __html: \\`if (typeof require !== 'undefined') window.ipcRef = require('electron').ipcRenderer;\\` }} />\\\n</body>^\" .cache/default-html.js > src/html.js"
  );
}
