...
const selectContentTypes = async () => {
  ...
  if (contentTypes.length === 0) {
    console.log(
      chalk.yellow('Warning - No content types exist in your Space. Exiting...')
    );
    process.exit(1);
  }
  ...
  rl.question('Enter content type id: ', answer => {
    ...
    console.log(
      chalk.green(`Creating template for ${selectedConfig.templateName}...`)
    );
    ...
    console.log(
      `Template index: ${chalk.green(
        `./src/templates/${selectedConfig.templateName}/index.js`
      )}`
    );

    // Copy boilerplate template into new template folder
    execSync(
      `cp -f ./scripts/contentful/config-files/contentful-template-boilerplate.js ./src/templates/${selectedConfig.templateName}/index.js`
    );

    // Replace placeholders in template page
    execSync(
      `sed -i '' "s/MyContentType/${selectedConfig.templateName}/g" ./src/templates/${selectedConfig.templateName}/index.js`
    );
    ...
    execSync(
      'cp -f ./scripts/contentful/config-files/contentful-gatsby-node.js ./gatsby-node.js'
    );

    // Replace placeholders in gatsby-node.js
    execSync(
      `sed -i '' "s/MyContentType/${selectedConfig.templateName}/g" ./gatsby-node.js`
    );

    console.log(chalk.green('Done!'));
    console.log(
      chalk.white(
        `Pages will now be generated for every record of type ${chalk.green(
          selectedConfig.id
        )}.\nTo confirm, run ${chalk.green(
          'yarn develop'
        )} and go to ${chalk.green(
          'http://localhost:3000/x/'
        )}. On the dev 404 page, you can view a list of all pages.`
      )
    );
