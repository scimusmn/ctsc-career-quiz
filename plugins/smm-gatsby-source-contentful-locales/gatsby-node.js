...
exports.sourceNodes = async (
  ...
  if (!options.spaceId)
    return reporter.error(`🚨  ${name}:spaceId is a required option `);
  if (!options.accessToken)
    return reporter.error(`🚨  ${name}:accessToken is a required option `);
  ...
  try {
    ...
    if (response.status === 404)
      throw new Error(`${data.message} Check if the spaceId is correct.`);
    // Throw error for 401, invalid authentication
    if (response.status === 401)
      throw new Error(`${data.message} Check if the accessToken is correct.`);
